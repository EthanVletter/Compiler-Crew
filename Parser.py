                ...
                if item.head == grammar.start_symbol:
                    ...
                    for a in follow[item.head]:
                        if (state_index, a) in action:
                            ...
                            if existing[0] == "shift":
                                conflicts.append(f"State {state_index}, symbol '{a}': Shift-Reduce conflict resolved in favor of shift")
                                continue
...
# ========================
# SPL Grammar (patched with ALGO → INSTR_LIST)
# ========================
def build_spl_grammar():
    productions = {
        ...
        # Algorithm (sequence of instructions) – patched
        "ALGO": [["INSTR_LIST"]],
        "INSTR_LIST": [
            ["INSTR"],
            ["INSTR",";","INSTR_LIST"],
